...
namespace res {
  ...
  template <typename T>	ION_API std::shared_ptr<T> CreateAsset(std::filesystem::path path);
  template <typename T>	ION_API void SaveAsset(std::filesystem::path path, std::shared_ptr<T> asset);
  template <typename T>	ION_API std::shared_ptr<T> LoadAsset(std::filesystem::path path, bool is_hash = true);
  template <typename T> ION_API void DestroyAsset(std::shared_ptr<T> asset);

  template <> ION_API std::shared_ptr<World> CreateAsset<World>(std::filesystem::path path);
  template <> ION_API void SaveAsset<World>(std::filesystem::path path, std::shared_ptr<World> asset);
  template <> ION_API std::shared_ptr<World> LoadAsset<World>(std::filesystem::path path, bool is_hash);
  template <> ION_API std::shared_ptr<Texture> LoadAsset<Texture>(std::filesystem::path path, bool is_hash);
  template <> ION_API std::shared_ptr<Shader> LoadAsset<Shader>(std::filesystem::path path, bool is_hash);
  template <>	ION_API std::shared_ptr<GPUData> LoadAsset<GPUData>(std::filesystem::path path, bool is_hash);
	template <> ION_API void SaveAsset<GPUData>(std::filesystem::path path, std::shared_ptr<GPUData> asset);
}
}