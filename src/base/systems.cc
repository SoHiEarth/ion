...
void ion::systems::UpdateSystems(std::shared_ptr<World>& world, UpdatePhase phase) {
	for (const auto& system : internal::systems) {
		if (system.phase == phase && system.enabled) {
			if (system.condition == UpdateCondition::ALWAYS ||
				(system.condition == UpdateCondition::WHEN_PLAYING && internal::is_playing) ||
				(system.condition == UpdateCondition::WHEN_STOPPED && !internal::is_playing)) {
...
bool ion::systems::GetState() {
	return internal::is_playing;
}

void ion::systems::SetState(bool playing) {
	internal::is_playing = playing;
}