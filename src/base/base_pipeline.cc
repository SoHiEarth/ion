...
BasePipeline::BasePipeline() {
  color_buffer = ion::render::CreateFramebuffer(FramebufferInfo{
    .recreate_on_resize = true,
    .name = "Color"
    });
  normal_buffer = ion::render::CreateFramebuffer(FramebufferInfo{
    .recreate_on_resize = true,
    .name = "Normal"
    });
  shaded = ion::render::CreateFramebuffer(FramebufferInfo{
    .recreate_on_resize = true,
    .name = "Shaded"
    });
  bloom_buffer = ion::render::CreateFramebuffer(FramebufferInfo{
    .recreate_on_resize = true,
    .name = "Bloom"
    });
  bloom_buffer_2 = ion::render::CreateFramebuffer(FramebufferInfo{
    .recreate_on_resize = true,
    .name = "Bloom 2"
    });
  deferred_shader = ion::res::LoadAsset<Shader>("assets/deferred_shader", false);
  ...
  bloom_blur_shader = ion::res::LoadAsset<Shader>("assets/bloom_blur_shader", false);
  combine_shader = ion::res::LoadAsset<Shader>("assets/bloom_combine_shader", false);
...
void BasePipeline::Render(std::shared_ptr<World> world, const PipelineSettings& settings) {
