...
void ion::physics::Update(std::shared_ptr<World>& world) {
    ...
    if (physics_body->enabled && b2Body_IsValid(physics_body->body_id)) {
      ...
      if (transform) {
        ...
        float body_rotation = b2Rot_GetAngle(b2Body_GetRotation(physics_body->body_id));
        if (transform->position.x != body_position.x || transform->position.y != body_position.y ||
            transform->rotation != body_rotation) {
          // Update physics body to match transform
          b2Body_SetAwake(physics_body->body_id, true);
          b2Body_SetTransform(physics_body->body_id,
          ...
          printf("Entity Transform Sync: ID %u, Position (%.2f, %.2f), Rotation %.2f\n",
                 entity, transform->position.x, transform->position.y, transform->rotation);
        }
      }
    }
  }
  ...
  for (auto& [entity, physics_body] : world->GetComponentSet<PhysicsBody>()) {
    if (physics_body->enabled && b2Body_IsValid(physics_body->body_id)) {
      ...
      if (transform) {
        ...
        transform->rotation = b2Rot_GetAngle(b2Body_GetRotation(physics_body->body_id));
				printf("Entity Physics Update: ID %u, Position (%.2f, %.2f), Rotation %.2f\n",
               entity, transform->position.x, transform->position.y, transform->rotation);
      }
    }
    if (physics_body->enabled && !b2Body_IsAwake(physics_body->body_id)) {
      b2Body_SetAwake(physics_body->body_id, true);
		...
		if (physics_body->enabled == false && b2Body_IsAwake(physics_body->body_id)) {
...
b2BodyId ion::physics::CreateBody(const std::shared_ptr<Transform>& transform) {
  ...
  b2Polygon shape = b2MakeBox(transform->scale.x * 0.5F, transform->scale.y * 0.5F);
...
bool ion::physics::BodyIsValid(b2BodyId body) {
